...
void generate_all_plans(int current_time, int *plan_number, int *current_plan, int plan_length) {
    if (current_time >= term) {
        ...
        for (int i = 0; i < plan_length; i++) {
            if (i > 0) fprintf(output_file, " $\\rightarrow$ ");
...
void generate_latex_report() {
    ...
    fprintf(output_file, 
        ...
        "Aqui va toda la yappeada del que es el replacement problem\n\n"
		"$C_{tx}$ is defined as the cost of buying equipment at time $t$ and selling it at time $x$:\n"
        "$$C_{tx} = \\text{Initial Cost} + \\sum_{i=1}^{x-t} \\text{Maintenance}_i - \\text{Resale}_{x-t}$$\n\n"
        "The Bellman equation is:\n"
        "$$G(t) = \\min \\{C_{tx} + G(x)\\}$$\n"
        "where $G(t)$ is the minimum cost from time $t$ to the end of the project.\n\n"
        "\\newpage\n"
        "\\subsection*{Problem Formulation}\n"
        "Given:\n"
        "\\begin{itemize}\n"
        "\\item Initial equipment cost: %d\n"
        "\\item Project term: %d years\n"
        "\\item Equipment useful life: %d years\n"
        "\\item Maintenance costs for each year of equipment life\n"
        "\\item Resale prices for each year of equipment life\n"
        "\\end{itemize}\n\n"
